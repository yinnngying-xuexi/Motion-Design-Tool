<template>
  <main class="app-shell" :class="{ 'home-mode': isHome, 'editor-mode': !isHome }">
    <header class="app-toolbar">
      <button class="toolbar-brand" type="button" aria-label="返回动效库首页" @click="selectModule('home')">
        <span>DataMotion Studio</span>
      </button>
      ...
      <nav class="top-navigation" aria-label="主导航">
        <button v-for="item in navItems" :key="item.key" type="button"
          ...
          @click="selectModule(item.key)">{{ item.label }}</button>
      </nav>
      <div v-if="isEditorModule" class="toolbar-search">
        <el-input
      ...
      <div v-if="isEditorModule" class="toolbar-actions">
...
type ModuleKey = "home" | "motion" | "decoration" | "my-motion";
